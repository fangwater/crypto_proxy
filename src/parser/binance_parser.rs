...
impl BinanceKlineParser {
    pub fn new( is_future: bool) -> Self {
        Self {
            is_future : is_future,
            http_client: if is_future { Some(reqwest::Client::new()) } else { None },
        }
    }
}
                ...
                if let Some(symbol) = json_value.get("s").and_then(|v| v.as_str()) {
                    ...
                    if let Some(kline_obj) = json_value.get("k") {
                        ...
                        if let (Some(open_str), Some(high_str), Some(low_str), Some(close_str),
                                 ...
                                 Some(trade_num), Some(taker_buy_vol_str), Some(taker_buy_quote_vol_str)) = (
                            ...
                            // 解析价格和成交量数据
                            if let (Ok(open), Ok(high), Ok(low), Ok(close), Ok(volume), Ok(turnover), Ok(taker_buy_vol), Ok(taker_buy_quote_vol)) = (
                                open_str.parse::<f64>(),
                                high_str.parse::<f64>(),
                                low_str.parse::<f64>(),
                                close_str.parse::<f64>(),
