...
impl CryptoProxyApp {
    pub async fn new(config: &'static Config) -> Result<Self> {
        info!(
            "Initializing CryptoProxyApp for exchange: {}",
            config.get_exchange()
        );
        ...
        let mkt_manager = Some(
            MktDataConnectionManager::new(config, &global_shutdown_tx, unified_tx.clone()).await,
        );

        info!("Initializing kline data manager");
        let kline_manager = Some(
            KlineDataConnectionManager::new(config, &global_shutdown_tx, unified_tx.clone()).await,
        );
        ...
        let derivatives_manager = match config.get_exchange().as_str() {
            "binance-futures" | "okex-swap" | "bybit" => {
                info!(
                    "Initializing derivatives metrics manager for {}",
                    config.get_exchange()
                );
                Some(
                    DerivativesMetricsDataConnectionManager::new(
                        config,
                        &global_shutdown_tx,
                        unified_tx.clone(),
                    )
            ...
            _ => {
                info!(
                    "Skipping derivatives metrics manager for spot exchange: {}",
                    config.get_exchange()
                );
                None
            }
    ...
    async fn shutdown_all_connections(&mut self) -> Result<()> {
        ...
        if let Some(ref mut manager) = self.derivatives_manager {
            if let Err(e) = manager.shutdown(&self.global_shutdown_tx).await {
                error!(
                    "Failed to shutdown DerivativesMetricsConnectionManager: {}",
                    e
                );
                return Err(anyhow::anyhow!(
                    "DerivativesMetricsConnectionManager shutdown failed"
                ));
            }
        }
    ...
    async fn update_all_subscribe_msgs(&mut self) -> Result<()> {
        if let Some(ref mut manager) = self.mkt_manager {
            if let Err(e) = manager.update_subscribe_msgs().await {
                error!(
                    "Failed to update subscribe msgs for MktConnectionManager: {}",
                    e
                );
                return Err(anyhow::anyhow!(
                    "MktConnectionManager update_subscribe_msgs failed"
                ));
            }
        }
        ...
        if let Some(ref mut manager) = self.kline_manager {
            if let Err(e) = manager.update_subscribe_msgs().await {
                error!(
                    "Failed to update subscribe msgs for KlineConnectionManager: {}",
                    e
                );
                return Err(anyhow::anyhow!(
                    "KlineConnectionManager update_subscribe_msgs failed"
                ));
            }
        }
        ...
        if let Some(ref mut manager) = self.derivatives_manager {
            if let Err(e) = manager.update_subscribe_msgs().await {
                error!(
                    "Failed to update subscribe msgs for DerivativesMetricsConnectionManager: {}",
                    e
                );
                return Err(anyhow::anyhow!(
                    "DerivativesMetricsConnectionManager update_subscribe_msgs failed"
                ));
            }
        }
